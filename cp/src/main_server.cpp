...
zmq::socket_t serverControlSocket(serverContext, ZMQ_REP); // Сокет для управления (REP-сокет)
 ...
 * socket - исходный сокет
 * Возвращает: принятую строку или сообщение об ошибке
 */
...
/*
 * Класс игрока:
 * Содержит логику игрового поля и взаимодействия с игроком
 */
class SeaBattlePlayer 
{
    public:
         ...
         * playerSocket - сокет для общения с клиентом
         * Логика:
         * 1. Для кораблей размером от 1 до 4
         * 2. Запрашивает координаты у игрока
         * 3. Проверяет валидность размещения
         * 4. Отправляет обновленное поле
         */
...
class SeaBattleGame 
{
    public:
         ...
         * 1. Поочередно дает ходы игрокам
         * 2. Проверяет попадания
...
int main() 
{
    ...
    // Привязываемся
    serverControlSocket.bind("tcp://*:5555");  // Основной порт управления
    firstPlayerSocket.bind("tcp://*:5556");    // Порт игрока 1
    secondPlayerSocket.bind("tcp://*:5557");   // Порт игрока 2
