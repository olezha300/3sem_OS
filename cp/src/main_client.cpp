// Реализует взаимодействие с сервером и игровую логику для пользователя
...
pthread_mutex_t commandMutex;     // Мьютекс для синхронизации доступа к командам
zmq::context_t zmqContext(2);     // Контекст ZeroMQ с 2 потоками
zmq::socket_t playerSocket(zmqContext, ZMQ_REP);  // Сокет для обмена сообщениями с сервером (REP-сокет)

int playerId;
std::string userCommand;

/* 
 * Функция отправки сообщения через ZeroMQ сокет
 * socket - целевой сокет
...
/*
 * Функция приема сообщения через ZeroMQ сокет
 * socket - сокет для приема сообщений
 * Возвращает: принятое сообщение или сообщение об ошибке
 */
...
/*
 * Формирование строки подключения к сокету
 * port - номер порта
 * Возвращает: строку вида "tcp://127.0.0.1:port"
 */
...
/*
 * Функция для проверки входящих приглашений в отдельном потоке
 * param - указатель на параметры (не используется)
 */
void* checkInvite(void *param) 
{
    std::string inviteTempString;
    pthread_mutex_lock(&commandMutex); // Блокируем мьютекс
    ...
    if (inviteTempString == "invite") 
    {
        ...
        else 
        {
            ...
            pthread_mutex_unlock(&commandMutex); // Разблокируем мьютекс
...
int main(int argc, char** argv) 
{
    ...
    zmq::context_t contextLocal(2);  // Локальный контекст ZeroMQ
    zmq::socket_t serverSocket(contextLocal, ZMQ_REQ);  // Сокет для связи с сервером
    ...
    CheckInviteParams checkInviteParams;  // Параметры для потока (пустые)
    pthread_t inviteThread;               // Идентификатор потока

    int processId = getpid();             // PID процесса
    std::string serverResponse;           // Ответ сервера
    std::string tempString;               // Временная строка
    ...
    while(true) 
    {
        ...
        if (iteration == 1) 
        {
            ...
            if (tempString == "Ok") 
            {
                ...
                std::getline(ss, tempString, ':');  
                playerId = std::stoi(tempString);     // Получаем назначенный ID
